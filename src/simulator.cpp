...
int Simulator::compute_alu(const RS_Entry &e, int vj, int vk) {
...
bool Simulator::eval_branch(const RS_Entry &e, int vj, int vk) {
  switch (e.funct3) {
  case 0x0:
    return vj == vk; // beq
  case 0x1:
...
bool Simulator::is_halted() const { return cur_halted && rob.is_empty(); }

uint32_t Simulator::get_result() const { return cur_reg[10] & 0xff; }

int Simulator::get_cycle_count() const { return cycle_count; }